...
module.exports = {
    // 查询所有借还记录
    findAll() {
        var sql = "select tb.Book_name,ts.Student_name,tbs.Id,tbs.Student_code,tbs.Book_num,tbs.borrow_date,tbs.return_date from t_book as tb,t_student as ts,t_book_student as tbs where tb.Book_num=tbs.Book_num and ts.username=tbs.username order by tbs.borrow_date desc";
        return pool.execute(sql);
    },
    // 学生查询借阅记录
    findRecords(name) {
        var sql = "select t_book_student.Book_num,Book_name,Student_name,borrow_date,return_date from t_book,t_student,t_book_student where t_book.Book_num=t_book_student.Book_num and t_student.username=t_book_student.username and t_book_student.username='" + name + "'";
        console.log(sql);
        return pool.execute(sql);
    },
    // 按书名模糊查询
    ...
    async save1(book) {
        ...
        var sql = "insert into t_book_student(Book_num,username,borrow_date,return_date) values('" + book.Book_num + "','"
            + book.username + "','"
            + book.borrow_date + "','"
            + book.return_date + "')";
    ...
    // 学生归还图书
    returnBook(id, name) {
        var sql = "delete from t_book_student where username='" + name + "' and Book_num='" + id + "'";
        console.log(sql);
        return pool.execute(sql);
    }
} 